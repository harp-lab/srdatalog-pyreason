...
## PyReason GPU companion submodule

This fork pins the companion
[`harp-lab/pyreason-gpu`](https://github.com/harp-lab/pyreason-gpu) prototype as
`third_party/pyreason-gpu`. Its `pyreason_gpu` implementation, formal semantics,
and SRDatalog-emission experiments remain owned and versioned in that repository;
this repository records only the tested commit.

Clone with the companion repository populated:

```bash
git clone --recurse-submodules https://github.com/harp-lab/srdatalog-pyreason.git
cd srdatalog-pyreason
```
